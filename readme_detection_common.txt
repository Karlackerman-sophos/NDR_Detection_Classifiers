Okay, I understand completely. You need the full README text with all the GitHub Flavored Markdown (GFM) syntax explicitly written out, so you can copy and paste it directly into a new file on GitHub, and it will render exactly as intended.

Here is the proposed README.md content:
...
### 3.2 External Git Repository Structure (`your-detection-rules-repo/`)

your-detection-rules-repo/
├── detection-rules/                 # Configured as GIT_RULES_PATH_VAR in config_map
│   ├── rule_id_1/                   # Directory named after the rule's unique ID
│   │   ├── metadata.json            # Rule metadata (JSON format)
│   │   └── query.sql                # Rule SQL query (ClickHouse SQL)
│   ├── rule_id_2/
│   │   ├── metadata.json
│   │   └── query.sql
│   └── ... (more detection rule directories)
├── README.md
├── .gitignore
└── ...
...
DB_USERNAME_VAR	ClickHouse database username	default
...
SYSLOG_IP_STR	IP address of the syslog server (for exclusion)	192.168.1.1
MANAGEMENT_IP_STR	IP address of the management interface (for exclusion)	10.0.0.1
...
BATCH_ID	A unique ID for the current execution batch (for temp dir)	20250725_1120_batchA
...
Creating metadata.json inside this new directory. This file defines the rule's name, description, frequency, execution device, etc., including an "enabled": true/false flag and "apply_global_ip_exclusions": true/false.

Creating query.sql inside this new directory. This file contains the ClickHouse SQL query defining the detection logic.

Use placeholders like {excluded_ips_list} in your WHERE clause if you want the global IP exclusions to be applied (this placeholder will be replaced with '' if apply_global_ip_exclusions is false for that rule, or with the actual IP list if true).
...
./detection_manager_test # Or whatever your executable is named (check your CMakeLists.txt)
